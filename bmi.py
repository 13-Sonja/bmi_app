...
class ResultText(ctk.CTkFrame):
    def __init__(self, parent, bmi_string, result_string):
        ...
        bmi_label = ctk.CTkLabel(self, font = bmi_font, text_color = WHITE, textvariable = bmi_string)
        bmi_label.pack(expand=True, fill="both", anchor="center")
        result_label = ctk.CTkLabel(self, font=result_font, text_color = WHITE, textvariable = result_string)
...
class WeightInput(ctk.CTkFrame):
    def __init__(self, parent, weight_float):
        ...
        label = ctk.CTkLabel(self, textvariable = self.weight_string, font = font, text_color = BLACK)
        label.grid(row = 0, column = 2)

        minus_button = ctk.CTkButton(self, command = lambda: self.update_weight(("minus", "large")), text = "-", font = font, text_color = BLACK, fg_color = LIGHT_GRAY, hover_color = GRAY, corner_radius = BUTTON_RADIUS)
        minus_button.grid(row = 0, column = 0, sticky = "ns", padx = 8, pady = 8)
        
        small_minus_button = ctk.CTkButton(self, command = lambda: self.update_weight(("minus", "small")), text = "-", font = font, text_color = BLACK, fg_color = LIGHT_GRAY, hover_color = GRAY, corner_radius = BUTTON_RADIUS)
        small_minus_button.grid(row = 0, column = 1, padx = 4, pady = 4)

        small_plus_button = ctk.CTkButton(self, command = lambda: self.update_weight(("plus", "small")), text = "+", font = font, text_color = BLACK, fg_color = LIGHT_GRAY, hover_color = GRAY, corner_radius = BUTTON_RADIUS)
        small_plus_button.grid(row = 0, column = 3, padx = 4, pady = 4)

        plus_button = ctk.CTkButton(self, command = lambda: self.update_weight(("plus", "large")), text = "+", font = font, text_color = BLACK, fg_color = LIGHT_GRAY, hover_color = GRAY, corner_radius = BUTTON_RADIUS)
...
class HeightInput(ctk.CTkFrame):
    def __init__(self, parent, height_int):
        ...
        slider = ctk.CTkSlider(self, variable = height_int, from_ = 100, to = 220, command = self.update_text, button_color = GREEN, button_hover_color = DARK_GREEN, progress_color = GREEN, fg_color = LIGHT_GRAY)
...
class AgeGenderInput(ctk.CTkFrame):
    def __init__(self, parent, age_group, gender):
        ...
        age_box = ctk.CTkComboBox(self, values=["19-24", "25-34", "35-44", "45-54", "55-64", "65+"], variable = self.age_var, command = lambda _: self.age_group.set(self.age_var.get()), state="readonly", font=font, hover=False, fg_color=WHITE, border_color=GREEN, button_color = GREEN, dropdown_fg_color=WHITE, dropdown_text_color=BLACK, text_color=BLACK, justify="center")
        ...
        female_box = ctk.CTkRadioButton(self, text="Female", variable = self.gender_var, command = lambda: self.gender.set(self.gender_var.get()), font = font, text_color = BLACK, border_color = GREEN, hover = False, fg_color = DARK_GREEN, border_width_checked=10, value="female")
        female_box.grid(row=1, column=2)
        male_box = ctk.CTkRadioButton(self, text="Male", variable = self.gender_var, command = lambda: self.gender.set(self.gender_var.get()), font = font, text_color = BLACK, border_color = GREEN, hover = False, fg_color = DARK_GREEN, border_width_checked=10, value="male")
